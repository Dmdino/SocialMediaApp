...
//  SocialMediaApp
//
//  Created by Дмитрий Папушин on 09/06/2019.
...
class EditProfileController: UIViewController {
    ...
    // do not forget to add this line of code "editProfileController.user = user" into UserprofileVC -> handleEditFollowTapped and editProfileController.userProfileController = self
    // it is nessesary to pass user date from UserProfileVC to EditProfileController I hade broken my head before I realize that.
...
// не работает, надо разобраться в чем причина
//Storage.storage().reference(forURL: user.profileImageUrl).delete(completion: nil)
//
//let filename = NSUUID().uuidString
//
...
//guard let imageData = updatedProfileImage.jpegData(compressionQuality: 0.3) else {return}
//
//STORAGE_PROFILE_IMAGES_REF.child(filename).putData(imageData, metadata: nil) { (metadata, err) in
//
//    if let err = err {
//        print("Failed to upload image to storage with error: ", err.localizedDescription)
//    }
