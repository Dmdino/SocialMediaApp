...
class UserProfileVC: UICollectionViewController, UICollectionViewDelegateFlowLayout, UserProfileHeaderDelegate {
    ...
    //MARK: - UIcollectionFlowLaout, seze for cell for posts in User Profile
    
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, minimumInteritemSpacingForSectionAt section: Int) -> CGFloat {
        return 1
    }
    // this two functions give me the one pixels of spaec between the cell
    ...
    //MARK - User profile Header Protokol
    ...
    func fetchPosts() {
        ...
        if currentKey == nil {
            USER_POSTS_REF.child(uid).queryLimited(toLast: 10).observeSingleEvent(of: .value) { (snapshot) in
                ...
                self.currentKey = first.key //.key represent a data snashot
            }
        } else {
            ...
            USER_POSTS_REF.child(uid).queryOrderedByKey().queryEnding(atValue: self.currentKey).queryLimited(toLast: 7).observeSingleEvent(of: .value) { (snapshot) in
                ...
                // do not forget to use it. It's set the cuurentKey to end point of snapshot
                self.currentKey = first.key
            }
        }
    }
    // helper func for fetchposts()
    ...
    func fetchCurrentUserData() {
        ...
        Database.database().reference().child("users").child(currentUid).observeSingleEvent(of: .value) { (snapshot) in
            ...
            let uid = snapshot.key // теперь снапшот содержить все значения пользователя
