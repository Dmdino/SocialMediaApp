...
extension UIViewController {
    ...
    func getMentionedUser(withUsername username: String) {
        ...
        USER_REF.observe(.childAdded) { (snapshot) in
            ...
            USER_REF.child(uid).observeSingleEvent(of: .value, with: { (snapshot) in
                // all snapshot values I've casted as a string
    ...
    func uploadMentionNotification(forPosId posId: String, withText text: String, isForComment: Bool ) {
                ...
                USER_REF.observe(.childAdded) { (snapshot) in
                    ...
                    USER_REF.child(uid).observeSingleEvent(of: .value, with: { (snapshot) in
                        ...
                        if word == dictionary["username"] as? String {
                            ...
                            let notificationValues = ["posId": posId, "uid": currentUid, "type": mentionIntegerValue!, "creationDate": creationDate] as [String: Any]
